    ...
    mutationFn: async (data: KpiSmartLiberatedFormData) => {
      ...
      const { data: newKpiSmartLiberated, error } = await supabase
        ...
        .insert({
          ...
          user_id: user.id,
          company_id: selectedCompany!.id, // Adicionado company_id
      ...
      <Card className="bg-card backdrop-blur-md border border-border shadow-lg rounded-2xl">
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Button onClick={() => navigate('/ops/shift/kpi-smarts-liberated/new')} className="bg-sollux-red hover:bg-sollux-red/90 text-white rounded-lg">
            <Plus className="mr-2 h-4 w-4" /> {isEditing ? 'Salvar Alterações' : 'Liberar KPI Smart'}
          </Button>
        </CardHeader>
        <CardContent>
          <Form {...form}>
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Select onValueChange={(value) => {
                      ...
                      setSelectedPillarIdForForm(value); // Atualiza o estado para filtrar KPIs
