...
import { ContractType, ContractTypeFormData } from '@/types/contractType';
...
const ContractTypesPage: React.FC = () => {
  ...
  const { data: items, isLoading } = useQuery<ContractType[], Error>({
    queryKey: ['contractTypes', user?.id],
    queryFn: async () => {
      if (!user?.id) return [];
      const { data, error } = await supabase.from('contract_types').select('*').eq('user_id', user.id).order('name');
  ...
  const mutationOptions = {
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['contractTypes', user?.id] });
  ...
  const createMutation = useMutation({
    mutationFn: async (data: ContractTypeFormData) => {
      ...
      const { error } = await supabase.from('contract_types').insert({ ...data, user_id: user.id });
      if (error) throw error;
    },
    ...mutationOptions,
    onSuccess: () => {
      mutationOptions.onSuccess();
      showSuccess('Tipo de Contrato criado com sucesso!');
    },
  });
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: ContractTypeFormData) => {
      ...
      const { error } = await supabase.from('contract_types').update(data).eq('id', editingItem.id);
      if (error) throw error;
    },
    ...mutationOptions,
    onSuccess: () => {
      mutationOptions.onSuccess();
      showSuccess('Tipo de Contrato atualizado com sucesso!');
    },
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      mutationOptions.onSuccess();
      showSuccess('Tipo de Contrato excluído com sucesso!');
    },
  });
    ...
    <div className="space-y-6">
      <Card className="bg-sollux-card-bg backdrop-blur-md border border-sollux-card-border shadow-lg rounded-2xl">
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="text-sollux-black uppercase font-bold">Gerenciar Tipos de Contrato</CardTitle>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-lg bg-sollux-card-bg backdrop-blur-md rounded-2xl shadow-lg border border-sollux-card-border">
          <DialogHeader><DialogTitle className="text-sollux-black">{editingItem ? 'Editar' : 'Novo'} Tipo de Contrato</DialogTitle></DialogHeader>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              <FormField control={form.control} name="name" render={({ field }) => (<FormItem><FormLabel className="text-sollux-black">Nome</FormLabel><FormControl><Input placeholder="Ex: CLT" {...field} className="rounded-lg" /></FormControl><FormMessage /></FormItem>)} />
