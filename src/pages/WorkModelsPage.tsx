...
import { WorkModel, WorkModelFormData } from '@/types/workModel';
...
const WorkModelsPage: React.FC = () => {
  const queryClient = useQueryClient();
  const { user } = useSession();
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<WorkModel | null>(null);

  const form = useForm<WorkModelFormData>({
  ...
  const { data: items, isLoading } = useQuery<WorkModel[], Error>({
    queryKey: ['workModels', user?.id],
    queryFn: async () => {
      if (!user?.id) return [];
      const { data, error } = await supabase.from('work_models').select('*').eq('user_id', user.id).order('name');
  ...
  const mutationOptions = {
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['workModels', user?.id] });
  ...
  const createMutation = useMutation({
    mutationFn: async (data: WorkModelFormData) => {
      if (!user?.id) throw new Error("Usuário não autenticado.");
      const { error } = await supabase.from('work_models').insert({ ...data, user_id: user.id });
      if (error) throw error;
    },
    ...mutationOptions,
    onSuccess: () => {
      mutationOptions.onSuccess();
      showSuccess('Modelo de Trabalho criado com sucesso!');
    },
  });
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: WorkModelFormData) => {
      ...
      const { error } = await supabase.from('work_models').update(data).eq('id', editingItem.id);
      if (error) throw error;
    },
    ...mutationOptions,
    onSuccess: () => {
      mutationOptions.onSuccess();
      showSuccess('Modelo de Trabalho atualizado com sucesso!');
    },
  });

  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from('work_models').delete().eq('id', id);
      if (error) throw error;
    },
    ...mutationOptions,
    onSuccess: () => {
      mutationOptions.onSuccess();
      showSuccess('Modelo de Trabalho excluído com sucesso!');
    },
  });

  const onSubmit = (data: WorkModelFormData) => {
    if (editingItem) {
